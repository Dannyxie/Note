...
- Some documents may be rewritten periodically but actually often contain the same data. The modification dates will change, even though the content hasn't
- Some documents may have changed, but only in ways that aren't important enough to warrant caches worldwide to reload the data
- Some servers cannot accurate determine the last modification dates of their pages.
- For servers that serve documents that change in sub-second intervals, the one-second granularity of modification dates might not be adequate


#Chpater 11 Client Identification and Cookies
##HTTP Headers
...
- A session cookie is a temporary cookie that keeps track of settings and preferences as a user navigates a site. A session cookie is deleted when the user exits the browser.
- Persistent cookies can live longer; they are stored on disk and survive browser exits and computer restarts. Persistent cookies often are used to retain a configuration profile or login name for a site that a user visits periodically.
- A cookie is a session cookie if its Discard parameter is set, or if there is no Expires or Max-Age parameter indicating an extended expiration time.
